...
#if defined(NDEBUG)
# error "Sfcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x604b21bcf2b03e1dd89cadc70d90b93b667f328939f6e723edc1f144fe5c28e9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sfcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sfcoin Signed Message:\n";
...
{
    // Sfcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	if(nHeight == 1)  
    {
        nSubsidy = 7000000 * COIN;  // premine on 3rd block
    }

	if(nHeight > 1000)  
    {
        nSubsidy = 1.5 * COIN;  // premine on 3rd block
    }
	if(nHeight > 3000)  
    {
        nSubsidy = 1 * COIN;  // premine on 3rd block
    }
	if(nHeight > 5000)  
    {
        nSubsidy = 0.5 * COIN;  // premine on 3rd block
    }
	if(nHeight > 15000)  
    {
        nSubsidy = 0.2 * COIN;  // premine on 3rd block
    }
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 100000); // Sfcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sfcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Sfcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sfcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sfcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x85f0fa19685c5af4506f26c62b448b136d352f4413f38f477d87ab6b222d117d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Doug Jones defeats Roy Moore in Alabama senate election 2018 - Walker";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0400082efb18114572a4056815ff018c40349403874fca0352473fc4938bb957449ebee1a6fb3d0bb7fae0987eb7326cfb949ccec21430ae1df15bf56e200121f4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1538928444;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085755886;

        if (fTestNet)
        {
            block.nTime    = 1538928425;
            block.nNonce   = 389141251;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xb928379747b6e2108ed1a28f47ae4102dd73e3faa9cf6a2022ff0dc5f1f734f0"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xcc, 0xb3, 0xd1 }; // Sfcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SfcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SfcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SfcoinMiner(CWallet *pwallet)
{
    printf("SfcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sfcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SfcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SfcoinMiner, pwallet));
}
